...
module.exports = {
    ...
    random: function (length, char, number) {
        length = length || 10;

        var map = '', result = '';

        if (char) map += 'abcdefghijklmnopqrstuvwxyz';
        if (number) map += '0123456789';
        if (this.nullOrEmpty(map)) map = 'abcdefghijklmnopqrstuvwxyz0123456789';
    ...
    params: function (params, encode) {
        encode = (encode === undefined) ? true : encode;

        var plist = [];

        _.each(params, function (param, key) {
            plist.push('{0}={1}'.f(key, encode ? encodeURIComponent(param) : param));
        });
    ...
    nullOrEmpty: function (s) {
        return (_.isString(s)) ? (s.trim() === '') : (s === undefined || s === null);
    }
};
